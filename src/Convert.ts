...
import {ConsumerResult, convert, LIMIT} from "./Globals";
...
export abstract class Convert{
    ...
    protected static reversed( value:string, radix:number, action:ConsumerResult<string,number>):number{
        ...
        for (; i >= 0; --i) {
            if (!Object.isNull(accepted = <number>action.accept(value[(len - 1) - i]))) {
    ...
    private static ConvertConsumerRounded = class ConvertConsumerRounded
         ...
         */
        get( ):string{
            return !(this.characteristics&0x1) ? super.get().replace(/^0*/,"")
    ...
    public static To = new class To implements convert{
        ...
        public int2Str( value:number, opts:ENDIAN = Convert.BIG_ENDIAN ):string{
            return Convert.convert(Object.requireNotNull(value), LIMIT.DB + 1, new Convert.ConvertIntToStr(opts) ).get();
        }
        /****
         * @base
         * @IOException
        ...
        public base(value:number, radix:number, consumer:ConsumerResult<number,string>):string{
            return Convert.convert(Object.requireNotNull(value), Object.requireNotNull(radix), consumer ).get();
        }
        /***
         * @binToNumber
         */
